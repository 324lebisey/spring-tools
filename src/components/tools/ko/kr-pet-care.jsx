...
export default function PetCareKR(){const[pt,sP]=useState("");const[ds,sDS]=useState("");const[ct,sCT]=useState("");const[petRegion,sPR]=useState("");const[con,sCon]=useState([]);const[plan,sPl]=useState(false);const[ch,sCh]=useState(()=>{try{const s=localStorage.getItem("kr-pet-care-checked");return s?JSON.parse(s):{};}catch{return{};}});const[exp,sExp]=useState({});const[allExp,sAE]=useState(false);const ref=useRef(null);
useEffect(()=>{try{localStorage.setItem("kr-pet-care-checked",JSON.stringify(ch));}catch{}},[ch]);
...
<button onClick={()=>{const secs=[...(hasDog?["vet-dog"]:[]),...(hasCat?["vet-cat"]:[]),...(hasDog?["groom-dog"]:[]),...(hasCat?["groom-cat"]:[]),...(hasDog?["home-dog"]:[]),...(hasCat?["home-cat"]:[]),...con.filter(c=>CONCERN_TASKS[c]).map(c=>`c-${c}`)];if(allExp){sExp({});sAE(false);}else{const a={};secs.forEach(s=>{a[s]=true;});sExp(a);sAE(true);}}} style={{width:"100%",marginBottom:12,padding:"10px",borderRadius:12,border:"2px solid #d7ccc8",background:"#efebe9",color:"#6d4c41",fontWeight:800,fontSize:14,cursor:"pointer",fontFamily:"inherit"}}>{allExp?"🔽 모두 접기":"🔼 모두 펼치기"}</button>
