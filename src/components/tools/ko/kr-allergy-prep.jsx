...
};const toggle=(id)=>setSymptoms(p=>p.includes(id)?p.filter(s=>s!==id):[...p,id]);const check=(k)=>setChecked(p=>({...p,[k]:!p[k]}));const gen=()=>{setPlan(true);setChecked({});setExpandedSym({[symptoms[0]]: true});setTimeout(()=>ref.current?.scrollIntoView({behavior:"smooth"}),150)};const rd=REGIONS.find(r=>r.id===region);const ok=region&&symptoms.length>0&&severity;
return(<div style={{minHeight:"100vh",background:"linear-gradient(160deg,#e8eaf6 0%,#e1f5fe 25%,#f1f8e9 55%,#fffde7 100%)",fontFamily:"'Noto Sans KR',sans-serif",position:"relative",overflow:"hidden"}}><link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;600;700;800&family=Jua&display=swap" rel="stylesheet"/>{["🌸","🤧","🌳","😷","🐝","🌼"].map((e,i)=>(<div key={i} style={{position:"absolute",top:`${5+i*15}%`,[i%2?"right":"left"]:`${2+(i%3)*2}%`,fontSize:22,opacity:0.1,pointerEvents:"none"}}>{e}</div>))}<div style={{maxWidth:780,margin:"0 auto",padding:"32px 20px",position:"relative",zIndex:1}}><div style={{textAlign:"center",marginBottom:36}}><div style={{fontSize:44,marginBottom:8}}>🤧🌸💊</div><h1 style={{fontFamily:"'Jua'",fontSize:"clamp(26px,5vw,38px)",color:"#5c6bc0",margin:"0 0 8px"}}>봄철 알러지 대비 플래너</h1><p style={{color:"#7986cb",fontSize:15,margin:0,fontWeight:600}}>꽃가루 + 황사 + 미세먼지 삼중고를 이겨내는 맞춤 플랜!</p></div>
<Box n="1" t="어디에 사시나요?" c="#5c6bc0"><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:10}}>{REGIONS.map(r=>(<B key={r.id} a={region===r.id} o={()=>setRegion(r.id)} c="#5c6bc0"><div style={{fontSize:22}}>{r.emoji}</div><div style={{fontWeight:800,fontSize:13}}>{r.name}</div></B>))}</div>{rd&&<div style={{marginTop:14,padding:"14px 18px",background:"#e8eaf6",borderRadius:14}}><div style={{fontWeight:800,color:"#3949ab",fontSize:14,marginBottom:8}}>📅 피크: {rd.peak}</div><div style={{fontSize:13,color:"#5c6bc0",fontWeight:600}}>주요 알러겐:</div>{rd.topAllergens.map((a,i)=>(<div key={i} style={{fontSize:13,color:"#666",marginTop:3,paddingLeft:12}}>• {a}</div>))}</div>}</Box>
<Box n="2" t="증상 선택 (복수 가능)" c="#5c6bc0"><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:10}}>{SYMPTOMS.map(s=>(<B key={s.id} a={symptoms.includes(s.id)} o={()=>toggle(s.id)} c="#5c6bc0"><span style={{fontSize:20}}>{s.emoji}</span><span style={{fontWeight:700,fontSize:12}}>{s.name}</span></B>))}</div></Box>
<Box n="3" t="심각도" c="#5c6bc0"><div style={{display:"flex",flexDirection:"column",gap:10}}>{SEVERITY.map(s=>(<B key={s.id} a={severity===s.id} o={()=>setSeverity(s.id)} c="#5c6bc0" w><span style={{fontSize:24,marginRight:12}}>{s.emoji}</span><div><div style={{fontWeight:800}}>{s.label}</div><div style={{fontSize:12,opacity:0.65}}>{s.desc}</div></div></B>))}</div></Box>
<Box n="4" t="약 복용 방식?" c="#5c6bc0"><div style={{display:"flex",flexDirection:"column",gap:10}}>{[{id:"before",label:"시즌 전부터 미리 복용",emoji:"⏰",desc:"피크 2-4주 전부터 시작 (가장 효과적!)"},{id:"during",label:"증상 나타나면 복용",emoji:"🤧",desc:"증상 기반 대응"},{id:"unsure",label:"잘 모르겠어요",emoji:"🤷",desc:"안내가 필요해요"}].map(t=>(<B key={t.id} a={timing===t.id} o={()=>setTiming(t.id)} c="#5c6bc0" w><span style={{fontSize:22,marginRight:12}}>{t.emoji}</span><div><div style={{fontWeight:800}}>{t.label}</div><div style={{fontSize:12,opacity:0.65}}>{t.desc}</div></div></B>))}</div></Box>
{ok&&timing&&<button onClick={gen} style={{width:"100%",padding:"16px",borderRadius:16,border:"none",background:"linear-gradient(135deg,#5c6bc0,#7986cb)",color:"#fff",fontFamily:"'Jua'",fontSize:20,cursor:"pointer",marginBottom:24,boxShadow:"0 4px 20px rgba(92,107,192,0.3)"}}>🌸 알러지 대비 플랜 만들기!</button>}
{plan&&<div ref={ref}>
<button
